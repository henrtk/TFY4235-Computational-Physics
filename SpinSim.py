...
def latticeChange(lattice, randnums, dt, xmax, ymax, C : Consts, notPeriodic):
    ez = np.array([0,0,1])
    paddingX =  notPeriodic or (xmax == 1)
    paddingY =  notPeriodic or (ymax == 1)

    tempLattice = np.zeros(shape = (ymax+paddingY,xmax+paddingX,3))
    res = np.zeros(shape = (ymax,xmax,3))
    if notPeriodic:
        tempLattice[:-1,:-1] = lattice
    else:
        if xmax == 1:
            tempLattice[:,:-1] = lattice
        elif ymax == 1:
            tempLattice[:-1,:] = lattice
        else: 
            tempLattice[:,:] = lattice
    ...
    for y in range(ymax):
        ...
        for x in range(xmax):
            ...
            spincoupling = J * (tempLattice[y,(x+1)%xmax] + tempLattice[y,x-1] + tempLattice[(y+1)%xmax,x] + tempLattice[y-1,x]) 
